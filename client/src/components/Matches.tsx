...
import { useProfileQuery, ProfileQueryVariables } from '../graphql/generated';
...
const MatchedProfile: React.FC<MatchedProfileProps> = ({ matchId }) => {
  ...
  return (
    <li className="bg-white rounded-lg shadow px-5 py-3 my-3 flex items-center justify-between">
      <div className="text-lg font-semibold text-gray-700">Match ID: {matchId}</div>
      <div className="text-sm text-gray-500">{data ? data?.profile?.name : 'Loading...'}</div>
    </li>
  );
};
...
const Matches: React.FC = () => {
  ...
  return (
    <div className="container mx-auto px-4">
      <h1 className="text-2xl font-bold text-gray-900 mt-10 mb-5">Your Matches</h1>
      {data?.allMatches && data.allMatches.length > 0 ? (
        <ul>
          {data.allMatches.map((matchId: any) => (
            <MatchedProfile key={matchId} matchId={matchId} />
          ))}
        </ul>
      ) : (
